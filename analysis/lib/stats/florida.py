...
fl_filename = src_dir / "fl_blueprint.tif"
...
def extract_by_geometry(geometries, bounds, prescreen=False):
    ...
    with rasterio.open(fl_filename) as src:
        try:
    ...
    counts = extract_count_in_geometry(
        fl_filename, shape_mask, window, np.arange(max_value + 1), boundless=True
    )
...
def summarize_by_aoi(shapes, bounds, outside_se_acres):
    """Get results for Florida Conservation Blueprint dataset
    ...
    values = pd.DataFrame(INPUTS["fl"]["values"])

    df = values.join(pd.Series(counts["fl"], name="acres"))
...
def get_huc12_results(id, analysis_acres, total_acres):
    """Get results for Florida Conservation Blueprint dataset for a given HUC12.
    ...
    row = df.loc[id]
    cols = [c for c in row.index if c.startswith("fl_")]
