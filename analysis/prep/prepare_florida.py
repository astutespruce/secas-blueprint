...
for id, indicator_filename in indicators.items():
    ...
    with rasterio.open(src_dir / "indicators" / indicator_filename) as src:
        data = extract_window(src, window, transform, nodata=nodata)
        max_value = data[data != src.nodata].max()

    # apply input area mask
    # note: this also wraps the incoming nodata value of -1 to 255
    data = np.where(mask == 1, data, nodata).astype("uint8")

    # flip the values so that they are ascending (highest value = priority 1)
    remap_table = np.array(
        [np.arange(1, max_value + 1), np.arange(max_value, 0, -1)], dtype="uint8"
    ).T
