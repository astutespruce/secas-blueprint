...
cols = [f"parca_{e['value']}" for e in PARCAS["values"]]
parcas = pd.read_feather(results_dir / "parcas.feather").set_index("id").join(huc12.rasterized_acres)
...
slr_nodata = encode_values(slr_results[SLR_NODATA_COLS], slr_results.rasterized_acres, 1000).rename("slr_nodata")
...
cols = [f"wildfire_risk_{e['value']}" for e in WILDFIRE_RISK["values"]]
wildfire_risk_results = (
    pd.read_feather(results_dir / "wildfire_risk.feather", columns=["id"] + cols)
    .set_index("id")
    .join(huc12.rasterized_acres)
)
wildfire_risk = encode_values(wildfire_risk_results[cols], wildfire_risk_results.rasterized_acres, 1000).rename(
    "wildfire_risk"
)
...
marine["subregions"] = marine.subregions.apply(lambda x: ",".join(str(subregions[s]) for s in x))


### Southeast Blueprint
# convert integer percents * 10, and pack into pipe-delimited string
print("Encoding Southeast Blueprint...")
blueprint_results = pd.read_feather(results_dir / "blueprint.feather").set_index("id").join(marine.rasterized_acres)
blueprint = encode_blueprint(blueprint_results)
...
cols = [f"protected_areas_{e['value']}" for e in PROTECTED_AREAS["values"]]
protected_areas_results = (
    pd.read_feather(results_dir / "protected_areas.feather").set_index("id").join(marine.rasterized_acres)
)
protected_areas = encode_values(protected_areas_results[cols], protected_areas_results.rasterized_acres, 1000).rename(
    "protected_areas"
)
...
out = pd.concat([huc12.reset_index(), marine.reset_index()], ignore_index=True, sort=False).reset_index(drop=True)


for col in ["protected_areas", "protected_areas_list", "urban"] + blueprint.columns.tolist() + slr.columns.tolist():
