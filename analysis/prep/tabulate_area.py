...
# only for those HUC12s that intersect with bounds of NatureScape dataset
tree = pg.STRtree(geometries)
ix = tree.query(pg.box(*NATURESCAPE_BOUNDS))
ns_geometries = geometries.iloc[ix]

index = []
results = []
for huc12, geometry in Bar(
    "Calculating NatureScape area for HUC12", max=len(ns_geometries)
).iter(ns_geometries.iteritems()):
    zone_results = extract_naturescape_area(
        [to_dict(geometry)], bounds=pg.total_bounds(geometry)
    )
    if zone_results is None:
        continue
...
results = results.reset_index()
results.to_feather(out_dir / "naturescape.feather")

if DEBUG:
    results.to_csv(huc12_debug_dir / "naturesscape.csv", index=False)
