...
results_filename = out_dir / "naturescape.feather"
...
def summarize_by_aoi(shapes, bounds, outside_se_acres):
    """Get results for NatureScape dataset
    ...
    values = pd.DataFrame(INPUTS["app"]["values"])

    df = values.join(pd.Series(results["app"], name="acres"))
    df["percent"] = 100 * np.divide(df.acres, total_acres)
    df["sort"] = [4, 0, 1, 2, 3]  # manually specified sort order
...
def get_huc12_results(id, analysis_acres, total_acres):
    ...
    row = df.loc[id]
    cols = [c for c in row.index if c.startswith("app_")]
